...
echo -e "\n" >> $LOG_PATH
echo "==========================" | tee -a $LOG_PATH
echo "Installation de PostgreSQL" | tee -a $LOG_PATH
echo "##########################" | tee -a $LOG_PATH
...
if [ "$POSTGRE_INSTALL_ERROR_EXIT_CODE" = "0" ]; then
	echo "Installation de PostgreSQL: OK" | tee -a $LOG_PATH
else
	echo "Erreur d'installation." | tee -a $LOG_PATH
	echo $POSTGRE_INSTALL_ERROR_EXIT_CODE >> $LOG_PATH
	exit 1
fi
...
echo "Initialisation de PostgreSQL" | tee -a $LOG_PATH
...
sudo -E -u postgres psql --command="CREATE DATABASE $DATABASE_NAME WITH ENCODING 'UTF8' LC_COLLATE='fr_FR.UTF-8' LC_CTYPE='fr_FR.UTF8' TEMPLATE=template0;"
sudo -E -u postgres psql --command="CREATE USER $DATABASE_USER WITH PASSWORD '$DATABASE_USER_PASSWORD';"
sudo -E -u postgres psql --command="GRANT ALL PRIVILEGES ON DATABASE $DATABASE_NAME to $DATABASE_USER;"
sudo -E -u postgres psql --command="ALTER DATABASE $DATABASE_NAME OWNER TO $DATABASE_USER;"
sudo -E -u postgres psql --command="GRANT USAGE, CREATE ON SCHEMA PUBLIC TO $DATABASE_USER;"


	#Changement du propriétaire du répertoire/var/lib/pgsql

PG_CONF_FILE=/var/lib/pgsql/data/pg_hba.conf
chown -vR $USERNAME /var/lib/pgsql | tee -a $LOG_PATH
mv -vf $PG_CONF_FILE $PG_CONF_FILE.bak | tee -a $LOG_PATH
...
read -p "Mattermost sera hébergé sur ce serveur ? (O/N)" OPEN_FIREWALL

if [ "$OPEN_FIREWALL" = "O" -or "$OPEN_FIREWALL" = "o"]; then
	echo "Ouverture du port du pare-feu"
	firewall-cmd --add-service=postgresql --permanent 2>> $LOG_PATH
